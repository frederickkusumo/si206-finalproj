...
API_KEY = "6f60ed46476634f335f15e251d488028f8277f15"
...
def create_population_dict():
    ...
    for num in range(0,len(cityData_list)):
        ...
        population = info[1][0]
        cityid_population_tup = (id_count,population)
        population_tup_list.append(cityid_population_tup)
        id_count += 1
...
def AV_create_database(population_tup_list,cur,conn):
    ...
    try:
        ...
        count = 0
    id = 1
    for item in population_tup_list[count:count+25]:
        var_id = id + count
        cur.execute("INSERT INTO CityPopulations (id,city_id,population) VALUES (?,?,?)",(var_id,item[0],item[1]))
        id += 1
    conn.commit()

def AV_csv_data(cur):
    cur.execute("SELECT city, population FROM CityPopulations JOIN Cities ON CityPopulations.city_id = Cities.id GROUP BY city_id ORDER BY city ASC LIMIT 25")
    rows = cur.fetchall()
    return(rows)

def AV_write_csv(data, filename):
    first_row = ["City", "Population"]
